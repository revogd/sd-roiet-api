...
app.get('/soldiers', (req, res) => {
    db.query("SELECT เลขประจำตัวประชาชน, ชื่อ, นามสกุล, เกิด, บ้านเลขที่, หมู่ที่, ตำบล, อำเภอ, จังหวัด, Datelogin,เหตุจำหน่าย FROM TB_Recuit WHERE อำเภอ='เมยวดี' AND เกิด='2566' ORDER BY เกิด ASC", (err, result) => {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
